<template>
  <div :class="['menu-wrapper', isCollapse ? 'change-item-style' : '']">
    <template v-for="item in menu">
      <el-menu-item v-if="validatenull(item[childrenKey]) && vaildRoles(item)" :index="item[pathKey]" :key="item[labelKey]" :class="{'is-active':vaildAvtive(item)}" @click="open(item)">
      ...
      <el-submenu v-else-if="!validatenull(item[childrenKey])&&vaildRoles(item)" :index="item[pathKey]" :key="item[labelKey]">
        ...
        <template v-for="(child,cindex) in item[childrenKey]">
          <el-menu-item v-show="child.hidden !== 1" v-if="validatenull(child[childrenKey])" :index="child[pathKey],cindex" :class="{'is-active':vaildAvtive(child)}" :key="child[labelKey]" @click="open(child)">
